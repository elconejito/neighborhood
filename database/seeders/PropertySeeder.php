...
class PropertySeeder extends Seeder
{
    ...
    private const OVERPASS_USER_AGENT = 'neighborhood-seeder/1.0 (contact: neighborhood@syde.app)';
    ...
    private function fetchAddresses(string $zip): array
    {
        ...
        try {
            $response = Http::timeout(90)
                ->withUserAgent(self::OVERPASS_USER_AGENT)
        ...
        if (! $response->successful()) {
            ...
            $bodySnippet = trim(preg_replace('/\s+/', ' ', mb_substr($response->body(), 0, 180)));
            $this->command->warn("Overpass request rejected for zip {$zip} (HTTP {$status}). Response: {$bodySnippet}");
